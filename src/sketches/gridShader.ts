...
function signedDepthToPill(px: number, py: number, c: CellRect): number {
  const halfW = c.w * 0.5;
  const halfH = c.h * 0.5;
  const radius = Math.min(halfW, halfH);
  const localX = px - (c.x + halfW);
  const localY = py - (c.y + halfH);
  const qx = Math.abs(localX) - (halfW - radius);
  const qy = Math.abs(localY) - (halfH - radius);
  const outside = Math.hypot(Math.max(qx, 0), Math.max(qy, 0));
  const inside = Math.min(Math.max(qx, qy), 0);
  return -(outside + inside - radius);
}

const CURSOR_BASE_W = 44;
/** Geometric-mean cell size that reads as 1× for cursor scale clamps. */
const CURSOR_SIZE_REF_PX = 300;

function edgeCursorScaleForCell(cr: CellRect): number {
  const sizeMul = Math.sqrt(cr.w * cr.h) / CURSOR_SIZE_REF_PX;
  return Math.min(4.0, Math.max(2.0, 3.0 * sizeMul));
}

/** Half-extent of the largest edge-scaled cursor reflection (px). */
function cursorReflectionOutsetPx(
  cr: CellRect,
  cursorImg: p5.Image | null
): number {
  const imgW = CURSOR_BASE_W * edgeCursorScaleForCell(cr);
  const aspect =
    cursorImg !== null
      ? cursorImg.height / Math.max(cursorImg.width, 1)
      : 1;
  const imgH = imgW * aspect;
  return Math.max(imgW, imgH) * 0.5;
}

function cursorScaleAtCell(
  px: number,
  py: number,
  cr: CellRect,
  plateau: number,
  ...
  const dSigned = signedDepthToPill(px, py, cr);
  const edgeScale = edgeCursorScaleForCell(cr);
  if (dSigned < 0) return edgeScale;
  const maxInPx = Math.max(Math.min(cr.w, cr.h) * 0.5, 1);
  const dNorm = Math.min(1, Math.max(0, dSigned / maxInPx));
  const tCurv = Math.max(
    0,
    Math.min(1, (dNorm - plateau) / Math.max(1 - plateau, 1e-4))
  );
  const side = Math.pow(1 - tCurv, flatPow);
  const sizeMul = Math.sqrt(cr.w * cr.h) / CURSOR_SIZE_REF_PX;
  const centerScale = Math.min(1.25, Math.max(1.0, 1.5 * sizeMul));
  return centerScale + (edgeScale - centerScale) * side;
}

/** True while the pointer is within the pill or the enlarged reflection footprint (pure SDF). */
function cursorReflectionActive(
  containerRects: CellRect[],
  px: number,
  py: number,
  cursorImg: p5.Image | null
): boolean {
  for (let i = 0; i < containerRects.length; i += 1) {
    const cr = containerRects[i]!;
    const dSigned = signedDepthToPill(px, py, cr);
    if (dSigned >= -cursorReflectionOutsetPx(cr, cursorImg)) return true;
  }
  return false;
}
...
export function createGridShaderSketch(
  ...
  return (p: p5) => {
    ...
    let defaultImg: p5.Image | null = null;
    ...
    const drawBackgroundLayer = () => {
      ...
      if (
        (cursorImg || defaultImg) &&
        cursorReflectionActive(d.containerRects, px, py, cursorImg)
      ) {
        ...
        for (let i = 0; i < d.containerRects.length; i += 1) {
          ...
          const dSigned = signedDepthToPill(px, py, cr);
          const outset = cursorReflectionOutsetPx(cr, cursorImg);
          if (dSigned < -outset || dSigned <= bestDepth) continue;
          bestDepth = dSigned;
          cursorScale = cursorScaleAtCell(px, py, cr, plateau, flatPow);
        }
        const insidePill = bestDepth >= 0;
        const reflectionImg = insidePill ? cursorImg : defaultImg;
        if (reflectionImg) {
          const imgW = CURSOR_BASE_W * cursorScale;
          const imgH = Math.round(
            imgW * (reflectionImg.height / Math.max(reflectionImg.width, 1))
          );
          bgLayer.push();
          bgLayer.tint(255, 90);
          bgLayer.image(
    ...
    p.setup = () => {
      ...
      if (!defaultImgLoadAttempted) {
        defaultImgLoadAttempted = true;
        p.loadImage(
          defaultImgUrl,
          (img) => { defaultImg = img; },
          () => { /* Default reflection unavailable — silently ignore. */ }
        );
      }
    };
      ...
      for (let i = 0; i < scene.containerRects.length; i += 1) {
        ...
        if (spin && spin.cellId === c.id) {
          ...
          if (pointerInCell) {
            ...
            // Use local pointer direction directly for convex-normal specular follow.
            specularXY = [
        ...
        dataRef.current.specDirByCellId[c.id] = [specX, specY];
